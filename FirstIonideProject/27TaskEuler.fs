...
let findBestCoefficientsRec limitA limitB maxPrime =
    ...
    let rec findBest a b maxCount maxA maxB =
        ...
        | _ when b > limitB ->
            findBest (a + 1) -limitB maxCount maxA maxB
        | _ ->
            let count = countPrimes a b 0 primeSet
            let (newMaxCount, newMaxA, newMaxB) =
                if count > maxCount then (count, a, b) else (maxCount, maxA, maxB)
...
let findBestCoefficientsList limitA limitB maxPrime =
    ...
    let countPrimesForAB a b =
        countPrimes a b 0 primeSet
    ...
    let (maxA, maxB, maxCount) =
        pairs
        |> List.fold (fun (maxA, maxB, maxCount) (a, b) ->
            let count = countPrimesForAB a b
            if count > maxCount then (a, b, count)
            else (maxA, maxB, maxCount)) (0, 0, 0)
